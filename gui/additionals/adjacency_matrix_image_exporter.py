...
class AdjacencyMatrixImageExporter:
    """
    Exports an adjacency matrix as a PNG image.
    """
    @staticmethod
    def export(matrix: List[List[int]], file_path: str, row_names: Optional[List[str]] = None, col_names: Optional[List[str]] = None):
        """
        Exports the given adjacency matrix as a PNG image.
        Args:
            matrix (List[List[int]]): The adjacency matrix to export.
            file_path (str): The path to the PNG file to write.
            row_names (Optional[List[str]]): Names for the rows (vertices).
            col_names (Optional[List[str]]): Names for the columns (vertices).
        """
        arr = np.array(matrix)
