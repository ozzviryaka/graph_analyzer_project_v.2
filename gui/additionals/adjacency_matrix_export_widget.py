...
from gui.additionals.adjacency_matrix_image_exporter import AdjacencyMatrixImageExporter
...
class AdjacencyMatrixExportWidget(QWidget):
    ...
    def export_png(self):
        ...
        if file_path:
            try:
                AdjacencyMatrixImageExporter.export(self.matrix, file_path, self.row_names, self.col_names)
                QMessageBox.information(self, 'Успіх', 'Матриця суміжності збережена у PNG!')
            except Exception as e:
                QMessageBox.critical(self, 'Помилка', str(e))
